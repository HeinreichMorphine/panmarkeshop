                    ...
                    <div class="md:w-1/2 p-6">
                        ...
                        <div class="flex space-x-4 mb-6">
                            <input type="number" id="quantity" value="1" min="1" max="${product.stock}" 
                                   class="w-20 px-3 py-2 border border-gray-300 rounded-md" ${product.stock <= 0 ? 'disabled' : ''}>
                            <button onclick="addToCart(${product.id}, parseInt(document.getElementById('quantity').value))" 
                                    class="${product.stock <= 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-panmark-accent hover:bg-opacity-90'} text-white px-6 py-2 rounded-md transition-colors" ${product.stock <= 0 ? 'disabled' : ''}>
