...
function getOrdersPageContent() {
    ...
    const orders = JSON.parse(localStorage.getItem('orders'));
    let userOrders = currentUser.role === 'admin' ? orders : orders.filter(order => order.userId === currentUser.username);
    const reviews = JSON.parse(localStorage.getItem('reviews'));

    // Sort admin orders by latest purchase (descending by date)
    if (currentUser.role === 'admin') {
                                ...
                                ${currentUser.role === 'admin' ? `
                                    <div class="mt-2">
                                        <select onchange="changeOrderStatus(${order.id}, this.value)" class="border border-gray-300 rounded px-2 py-1" ${order.status === 'Cancelled' || order.status === 'Received' ? 'disabled' : ''}>
                                            ${['Pending','Processed','Shipping','Cancelled','Received'].map(status => `
                                                <option value="${status}" ${order.status === status ? 'selected' : ''}>${status}</option>
                                            `).join('')}
                                        </select>
                                    </div>
                                ` : ''}
                                ${currentUser.role === 'customer' && order.status === 'Shipping' ? `
                                    <div class="mt-2">
                ...
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="text-gray-600">View all your orders in "Order History". Track status: Pending, Processed, Shipping, Received, or Cancelled.</p>
                        </div>
                        <div>
